...
- `BLOCKS`: For long-form text, "About Us" stories, or mixed media that doesn't fit the above, including price lists.

## Constraints
1. **Output ONLY the category name**. No other text.
2. **Be specific**: If it mentions "FAQ" or "collapsible", it is ALWAYS `ACCORDION`.
3. **Be specific**: If it mentions "tabs" or "categories", it is ALWAYS `TABS`.
4. **Be specific**: If it mentions "gallery" or "portfolio", it is ALWAYS `GALLERY`.
5. **Be specific**: If it mentions "money", "cost", "price", "packages", "service tiers", it is ALWAYS `BLOCKS`.
